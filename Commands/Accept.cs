...
{
    [PluginExport]
    public class Accept : IDiscordCommand
    {
        public HelpDetails GetCommandHelp()
        {
            return Utils.Init((ref HelpDetails h) =>
            {
                h.CommandName = "accept";
                h.ShortDescription = "Accepts a suggestion!";
            });
        }
        ...
        public async Task Handle(SocketMessage msg, SocketGuild guild, CommandHandler commandHandler)
        {
            ...
            AcceptSuggestion(int.Parse(msg.Content.Split(' ')[1]));
            if (success)
                await msg.Channel.SendOk("Suggestion accepted!");
        }
        ...
        private void AcceptSuggestion(int index)
        {
            ...
            if (Clubby.Program.config.DiscordChannels.TryGetValue("suggestions", out channel_id))
            {
                ...
                if (channel != null)
                {
                    ...
                    channel.SendMessageAsync(null, false, Suggest.ConstructAccept(suggestion, reason));
                }
            }
